...
abstract class Annotations
{
  ...
  Annotations(List<Line> lines) {
    this._lines = lines;
    this._timeToLevel = calculateColorMap(lines.map((e) => e.timestamp).toList());
  }

  /// Turn a set of timestamps into a gray scale.

  Map<int, int> calculateColorMap(List<int> timestamps)
  {
    ...
    {
      if (time == null)
        throw Exception("Null time in timestamps");
    ...
    for (int i=0; i<sorted.length; i++)
    {
      ...
      double t0 = (time.toDouble() - minTime) / (maxTime - minTime);
      double t1 = (i.toDouble()) / sorted.length;

      double t = t0 * t1 * t0 * t1 * t0 * t1;
      colorMap[time] = (t * 255).floor();
    }
